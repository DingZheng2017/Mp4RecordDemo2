...
public final class ExtAudioCapture {
   ...
   private byte[] mAudioSrcBuffer = new byte[SAMPLES_PER_FRAME ];

    private OnAudioFrameCapturedListener mOnAudioFrameCapturedListener;

    public interface OnAudioFrameCapturedListener {
        void onAudioFrameCaptured(byte[] audioData,int size);
    }
            ...
            while (!mIsLoopExit) {
                ...
                if (ret == AudioRecord.ERROR_INVALID_OPERATION) {
                    ...
                    if (mOnAudioFrameCapturedListener != null) {
                        mOnAudioFrameCapturedListener.onAudioFrameCaptured(mAudioSrcBuffer,ret);
                    }
                }
            }
        }
